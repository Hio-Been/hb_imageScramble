...
nSection = nSection + 1;
rowSpace = floor(linspace( 1, nRow , nSection ));
colSpace  = floor(linspace( 1, nCol , nSection ));
min_possibleSize = [ max(diff(rowSpace)), max(diff(colSpace)) ];

rowSpace(end) = nRow; colSpace(end) = nCol;

tempCell = cell([1,(nSection-1)^2]);
appendPermute = randperm( (nSection-1)^2 );
...
for rowIdx = 1:length(rowSpace)-1
    for colIdx = 1:length(rowSpace)-1
        nAppend = nAppend+1;
        tempCell{1,appendPermute(nAppend)} = imresize(inputImage(...
            rowSpace(rowIdx):rowSpace(rowIdx+1)-1,...
            colSpace(colIdx):colSpace(colIdx+1)-1, :), min_possibleSize);
    end
end

% CAT image 1 (integrate by rows)
cat_row = cell( [1, nSection-1] );
cat_row_nAppend = 0;
for cellIdx = 1:length(tempCell)
    if mod(cellIdx, nSection-1) == 1
...
% CAT image 2 (integrate by columns)
cat_col = [];
